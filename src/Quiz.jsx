...
function Quiz() {
  ...
  if (currentQuestionIndex >= quiz.gorseller.length) {
    ...
    if (userPuan == 100) {
      ...
      ekranSaati = "> 5.5 Saat";
    } else if (userPuan <= 90 && userPuan > 80) {
      //  80-90 arası
      developerMessage =
        "bağımlı olmana ramak kalmış dikkat et kardeş (sağlık için)";
      ekranSaati = "> 4 Saat";
    } else if (userPuan <= 80 && userPuan > 70) {
      // 70-80 arası
      developerMessage =
        "bağımlılık seziyorum haftaya olacak testte bundan yüksek çıkmasın (sağlığın icin)";
      ekranSaati = "> 3.5 saat";
    } else if (userPuan <= 70 && userPuan > 60) {
      //60-70 arası
      developerMessage =
        "twittera sık sık giriyorsun bence çünkü puanın hiç azımsanacak bir puan değil, bağımlı olma dikkat et <3";
      ekranSaati = "> 3 Saat";
    } else if (userPuan <= 60 && userPuan >= 50) {
      //50-60 arası
      developerMessage =
        "kılpayı testten geçtin ve twittera daha bağımlı olmamışsın gibi gözüküyor böyle devam et haftaya yine gel <3 ";
      ekranSaati = "> 2.5 Saat";
    } else if (userPuan <= 49 && userPuan > 40) {
      //40-50 arası
      developerMessage =
        "geçemediniz malesef, twitter süreni biraz daha arttırabilirsin site bağımlısı olma lütfen (ben gibi)";
      ekranSaati = "> 2 Saat";
    } else if (userPuan <= 40 && userPuan > 30) {
      //30-40 arası
      developerMessage =
        "geçemedin malesef, twitter süren az gözüküyor haftaya yine gel";
      ekranSaati = "> 1.5 Saat";
    } else if (userPuan <= 30 && userPuan > 20) {
      //20-30 arası
      developerMessage =
        "twittera az giriyorsun heralde bu puan hiç yüksek değil";
      ekranSaati = "1 Saat >";
    } else if (userPuan <= 20 && userPuan > 10) {
      //10-20 arası
      developerMessage =
        "twittera son zamanlarda az girmişsin süreyi arttırabilirsin gibi";
      ekranSaati = "30 Dakika >";
    } else if (userPuan <= 10 && userPuan > 0) {
      //0-10 arası
      developerMessage =
        "twittera aşırı az giriyorsun bu siteyi keşfetmen bile senin için mucize";
      ekranSaati = "15 Dakika >";
    } else if (userPuan == 0) {
      //0 puan
      developerMessage =
        "0 puan ne arkadaş siteyi nereden buldun twitter yerine facebooka giriyorsun heralde";
